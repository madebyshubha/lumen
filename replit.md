...
- **Repo cleanup done (Task #43)**: removed `artifacts/lumen-trailer/` (Remotion), `artifacts/lumen-landing/` (no source), `attached_assets/`, `screenshots/`, 4 zero-import components (`CapabilityBadges`, `KeyboardAwareScrollViewCompat`, `Stepper`, `TaskCard`), and `scripts/src/hello.ts` placeholder. `dist/` added to `.gitignore`.
